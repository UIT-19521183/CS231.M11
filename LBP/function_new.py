...
def read_images(pos_dir, neg_dir):
    ...
    for obj in objects:
      if obj['lbl'] == 'person': 
        ...
        if (cropped.shape[1]>0):
          ...
          hog_feature = hog(cv2.resize (cropped, (64, 128)), orientations = 9, pixels_per_cell=(8,8), cells_per_block=(2,2))
          feature_new = np.append(hist_lbp, hog_feature)
  ...
  for neg_image in os.listdir(neg_images_dir):
    ...
    for win in windows:
      ...
      feature1 = hog(cropped, orientations = 9, pixels_per_cell=(8,8), cells_per_block=(2,2))
      feature2 = np.append(feature, feature1)
