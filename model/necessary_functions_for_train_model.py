...
def get_FP_sample(X_test,y_test,pred):
    X_FP=[]
    y_FP=[]
    for i in range(len(X_test)):
        if(y_test[i]==1 and pred[i]==0):
            X_FP.append(X_test[i])
            y_FP.append(y_test[i])
    return X_FP,y_FP
def get_num_of_FN(X_test,y_test,pred):
    dem=0
    for i in range(len(X_test)):
...
def predict_(X_test_file,y_test_file,clf1,scaler):
    ...
    plt.show()
    X_FP,y_FP=get_FP_sample(X_test,y_test,pred)
    return X_FP,y_FP
...
def train_by_k_fold(X,y,n_split,classifier):
    ...
    for train_index, test_index in skf.split(X, y):
        ...
        X_TP_temp,y_TP_temp=get_FP_sample(x_test_fold,y_test_fold,y_pred)
        X_TP=X_TP+X_TP_temp
        y_TP=y_TP+y_TP_temp
        num_of_FN+=get_num_of_FN(x_test_fold,y_test_fold,y_pred)
        num_of_FP+=len(X_TP_temp)
...
def training(X_,y_,clf1,scaler):
    ...
    predict_(X_test_file,y_test_file,clf1,scaler)

    #X_FP,y_FP=train_by_k_fold(X_val,y_val,clf1,scaler2)
    #Lấy X_FP và y_FP
    X_FP,y_FP=train_by_k_fold(X_,y_,n_split=5,classifier=clf1)

    #Gộp train và FP vào để train lại
    X_train2=np.concatenate((X_, X_FP), axis=0)
    y_train2=np.concatenate((y_, y_FP), axis=0)

    #Train_data lần 2
    #clf1.fit(X_train2,y_train2)
    clf1.fit(X_train2,y_train2)
    joblib.dump(clf1, 'SVM_rbf_cell_size_8x8_FP.pkl')
